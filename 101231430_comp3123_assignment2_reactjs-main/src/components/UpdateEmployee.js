...
// eslint-disable-next-line        
    ...
    const getEmployeeDataByID = async () => {
        const result = await axios.get(`https://comp3123-assignment1-nodejs.herokuapp.com/api/emp/employees/${id}`);
        ...
        const onSubmit = (e) => {
            ...
            if (newEmployee.first_name.length <= 0 || newEmployee.last_name.length <= 0 || newEmployee.email_id.length <= 0) {
                ...
                axios.put(`https://comp3123-assignment1-nodejs.herokuapp.com/api/emp/employees/${id}`, newEmployee)
                .then(res => {
                    notifySuccess();
                    navigate('/employees');
                })
